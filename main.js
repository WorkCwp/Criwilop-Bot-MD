    ...
    if (pairingCode && !conn.authState.creds.registered) {
        ...
        if (!!phoneNumber) {
            ...
            if (!Object.keys(PHONENUMBER_MCC).some(v => numeroTelefono.startsWith(v))) {
                console.log(chalk.bgBlack(chalk.redBright("Comience con el código de país de su número de WhatsApp, Ejemplo: +573244488043")))
                process.exit(0)
            }
        } else {
            numeroTelefono = await question(chalk.bgBlack(chalk.greenBright(`Por favor, escriba su número de WhatsApp 😍\nPor ejemplo: +573244488043 : `)))
...
let prekey = []
let directorio = readdirSync("./CwpSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-') /*|| file.startsWith('session-') || file.startsWith('sender-') || file.startsWith('app-') */
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./CwpSession/${files}`)
})
} 
...
const directories = ['./CwpSession/', './jadibts/']
...
global.reloadHandler = async function(restatConn) {
  ...
  conn.sSubject = '*sᴇ ᴀʜ ᴄᴀᴍʙɪᴀᴅᴏ ᴇʟ ɴᴏᴍʙʀᴇ ᴅᴇʟ ɢʀᴜᴘᴏ*\n*𝙽𝚄𝙴𝚅𝙾 𝙽𝙾𝙼𝙱𝚁𝙴:* @subject';
  conn.sIcon = '*sᴇ ᴀʜ ᴄᴀᴍʙɪᴀᴅᴏ ʟᴀ ᴘᴏᴛᴀᴅᴀ ᴅᴇʟ ɢʀᴜᴘᴏ*';
  conn.sRevoke = '*ʟɪɴᴋ ᴅᴇʟ ɢʀᴜᴘᴏ ᴀᴄᴛᴜᴀʟɪᴢᴀᴅᴏ*\n*𝙻𝙸𝙽𝙺 𝙽𝚄𝙴𝚅𝙾:* @revoke';
