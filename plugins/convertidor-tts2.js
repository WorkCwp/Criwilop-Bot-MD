...
const handler = async (m, { conn, usedPrefix, command, text, args }) => {
  ...
  if (!efecto) {
    ...
    let responseText = `*⚠️ No haz ingresado un efecto, por favor ingresa un efecto de voz.*\n\n*—◉ Elige uno de los siguientes efectos:*\n`;
  ...
  if (!efectoValido) return conn.sendMessage(m.chat, { text: `*⚠️ El efecto proporcionado no existe en la lista, utiliza ${usedPrefix + command} para conocer la lista de efectos.*` }, { quoted: m });

  if (!texto) return conn.sendMessage(m.chat, {text: `*⚠️ Ingresa el texto que quieras convertir a audio.*\n\n*—◉ Ejemplo:*\n*◉ ${usedPrefix + command} ${efecto} Hola, este es un ejemplo de uso del comando.*`}, {quoted: m});
...
async function getVoiceList() {
  ...
  try {
    ...
    return { resultado: '⚠️ Error, no se obtuvo respuesta de la API.' };
    throw error;
  }
}
...
async function makeTTSRequest(texto, efecto) {
  ...
  try {
    ...
    return { resultado: url ? url : '⚠️ URL no encontrada en la respuesta.' };
  } catch (error) {
    console.error('Error:', error);
    return { resultado: '⚠️ Error, no se obtuvo respuesta de la API.' };
  }
