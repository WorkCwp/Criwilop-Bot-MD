...
const handler = async (m, {conn, command, args, text, usedPrefix}) => {
  if (!text) throw `_*< DESCARGAS - PLAY />*_\n\n*[ ℹ️ ] Hace falta el título o enlace del video de YouTube.*\n\n*[ 💡 ] Ejemplo:* _${usedPrefix + command} Good Feeling - Flo Rida_`;
    ...
    if (command == 'playy') {
    try {   
    const audio = `https://api.yanzbotz.my.id/api/downloader/ytmp3?url=${yt_play[0].url}`)
    ...
    if (command == 'playy2') {
    try {   
    const video = `https://api.yanzbotz.my.id/api/downloader/ytmp3?url==${yt_play[0].url}`)
