...
const handler = async (m, {text, usedPrefix, command, conn}) => {
  if (!text) throw '*⚠️ 𝙸𝙽𝙶𝚁𝙴𝚂𝙰 𝙴𝙻 𝙽𝙾𝙼𝙱𝚁𝙴 𝙳𝙴 𝙰𝙻𝙶𝚄𝙽𝙰 𝙿𝙴𝙻𝙸𝙲𝚄𝙻𝙰 𝙰 𝙱𝚄𝚂𝙲𝙰𝚁*';
  let aaaa;
  let img;
  try {
  ...
  if (aaaa == '') throw '*⚠️ 𝙽𝙾 𝚂𝙴 𝙴𝙽𝙲𝙾𝙽𝚃𝚁𝙾 𝙽𝙸𝙽𝙶𝚄𝙽𝙰 𝙿𝙴𝙻𝙸𝙲𝚄𝙻𝙰*';
...
const handler = async (m, { text, usedPrefix, command, conn }) => {
  if (!text) throw '*⚠️ Falta el nombre de la película o serie*'

  let result = await searchContent(text)
  if (command === 'pelisplus') result = await searchPelisPlus(text)

  if (result.length === 0) throw '*⚠️ No se encontró ningún contenido relacionado*'
