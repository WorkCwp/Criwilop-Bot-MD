...
let text
    if (args.length >= 1) {
        ...
        text = m.quoted.text;
    } else throw "*⚠️ ᴜsᴏ ɪɴᴄᴏʀʀᴇᴄᴛᴏ ᴅᴇʟ ᴄᴏᴍᴀɴᴅᴏ, ᴅᴇʙᴇ ᴇsᴄʀɪʙɪʀ ᴜɴ ᴛᴇxᴛᴏ*";
   if (!text) return m.reply('*⚠️ ᴜsᴏ ɪɴᴄᴏʀʀᴇᴄᴛᴏ ᴅᴇʟ ᴄᴏᴍᴀɴᴅᴏ, ᴅᴇʙᴇ ᴇsᴄʀɪʙɪʀ ᴜɴ ᴛᴇxᴛᴏ*');
    const who = m.mentionedJid && m.mentionedJid[0] ? m.mentionedJid[0] : m.fromMe ? conn.user.jid : m.sender; 
    const mentionRegex = new RegExp(`@${who.split('@')[0].replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*`, 'g');
    const mishi = text.replace(mentionRegex, '');
   if (mishi.length > 30) return m.reply('*⚠️ ᴇʟ ᴛᴇxᴛᴏ ɴᴏ ᴘᴜᴇᴅᴇ ᴛᴇɴᴇʀ ᴍᴀs ᴅᴇ 30 ʟᴇᴛʀᴀs *');
