...
const resolveRoulette = (chatId, conn) => {
    ...
    if (rouletteBets[chatId] && rouletteBets[chatId].length > 0) {
        ...
        for (let bet of rouletteBets[chatId]) {
            let result = '';
            if (colour === bet.color) {
                result = `@${bet.user.split('@')[0]} ganaste ${bet.amount}`;
                global.db.data.users[bet.user].credit += bet.amount;
                winners.push(result);
            } else {
                result = `@${bet.user.split('@')[0]} perdiste ${bet.amount}`;
                global.db.data.users[bet.user].credit -= bet.amount;
            }
        }
...
const betRoulette = (user, chatId, amount, color) => {
    ...
    return `✅ Tu apuesta de ${amount} oro en ${color} ha sido colocado!`;
};
...
//const handler = async (m, { conn, args, usedPrefix, command }) => {
    ...
    if (args.length < 2) {
        throw `✳️ Uso de comandos: ${usedPrefix + command} <cantidad> <color>\n\n Ejemplo: ${usedPrefix + command} 500 red`;
    }
