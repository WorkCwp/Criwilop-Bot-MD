...
${usedPrefix + command} 500*`.trim()
    ...
    if (new Date - users.lastslot < 10000) throw `⏳ Esperar *${msToTime(time - new Date())}* para usar de nuevo`
    ...
    let end;
    if (a == b && b == c) {
        end = `🎊 *¡Bote!* Ganaste ${amount + amount} oro`
        users.credit += amount + amount
   // } else if (a == b || a == c || b == c) {
   //     end = `Perdiste  *₹${amount}*\n*Pero casi lo logras, sigue intentándolo.*`
   //     users.credit -= amount
    } else {
        end = `      Perdiste ${amount} oro`
        users.credit -= amount
    }
